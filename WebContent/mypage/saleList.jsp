			...
			<div class="inner">
				...
				<section>
					...
					<ul class="pagination" style="text-align:center">
						...
						<%if(firstPage-1 > 1){%>
						<li><a href="/mypage/saleList?currentPage=<%=firstPage-1 %>" class="button">Prev</a></li>
				        <%}else{%>
				        <li><span class="button disabled">Prev</span></li>
				        <%}%>
						<% for(int i=firstPage; i<=lastPage; i++){ %>
        				<%if(i>totalPage) break; %>
        				<li><a <% if(currentPage==i){%>class="page active"<%}else{%>class="page"<%}%> href="/mypage/saleList?currentPage=<%=i%>"><%= i %></a></li>
        				<% } %>
						 <%if((lastPage+1)>totalPage) {%>
				        <li><span class="button disabled">Next</span></li>
				        <%}else{%>
				        <li><a href="/mypage/saleList?currentPage=<%=lastPage+1%>" class="button">Next</a></li>
				        <%}%>
					</ul>
				</section>
			</div>
